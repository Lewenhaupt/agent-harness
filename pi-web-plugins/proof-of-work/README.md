...
> The `playwright` binary and its browser set come from the Nix runtime env
> (`PLAYWRIGHT_BROWSERS_PATH`). See
> [docs/playwright-proof-env.md](../../docs/playwright-proof-env.md) for how to
> verify the devShell/pi-web toolchain and how to generate proofs with
> `playwright` / `playwright-cli` (always pass `--browser=chromium`).
...
| Screenshot (`.png`/`.jpg`) | playwright-cli | `playwright-cli open --browser=chromium URL → screenshot path.png` (both `playwright` and `playwright-cli` come from the Nix runtime env) |
